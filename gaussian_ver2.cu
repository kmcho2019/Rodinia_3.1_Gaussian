...
{   
	...
	*/
	// refractor code to make it more readable
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int row = idx + t + 1;

    if (idx >= Size - 1 - t) return;

    m_cuda[row * Size + t] = a_cuda[row * Size + t] / a_cuda[t * Size + t];
}
 ...
 */ 
 ...
 // new ver2 code takes advantage of shared memory
__global__ void Fan2(float *m_cuda, float *a_cuda, float *b_cuda,int Size, int j1, int t)
{
	// Shared memory allocation
	__shared__ float shared_a_r[BLOCK_SIZE_XY];
	__shared__ float shared_a_w[BLOCK_SIZE_XY][BLOCK_SIZE_XY];
	...
	if (Size*(xidx+1+t)+(yidx+t) < Size*Size)
	{
		shared_a_w[threadIdx.x][threadIdx.y] = a_cuda[Size*(xidx+1+t)+(yidx+t)];
	}
	else
	{
		shared_a_w[threadIdx.x][threadIdx.y] = 0.0;
	}
	if (Size*(t)+(yidx+t) < Size*Size)
	{
		shared_a_r[threadIdx.y] = a_cuda[Size*(t)+(yidx+t)];
	}
	else
	{
		shared_a_r[threadIdx.y] = 0.0;
	}
	__syncthreads();


	//a_cuda[Size*(xidx+1+t)+(yidx+t)] -= m_cuda[Size*(xidx+1+t)+t] * a_cuda[Size*t+(yidx+t)];
	shared_a_w[threadIdx.x][threadIdx.y] -= m_cuda[Size*(xidx+1+t)+t] * shared_a_r[threadIdx.y];
	__syncthreads();
	// write data back to global memory
	if (Size*(xidx+1+t)+(yidx+t) < Size*Size)
	{
		a_cuda[Size*(xidx+1+t)+(yidx+t)] = shared_a_w[threadIdx.x][threadIdx.y];
	}
	if(yidx == 0){
